...
Buffer Instancing::createBufferForBufferView(const tinygltf::Model &model,
...
void Instancing::setupPrimitive(const tinygltf::Model &model,
...
InstanceBindGroupAndCount Instancing::setupPrimitiveInstances(const PrimitiveKey &primitiveKey,
                                                              const PrimitiveInstances &primitiveInstances)
{
    const auto &instanceDataSet = primitiveInstances.at(primitiveKey);
    const uint32_t instanceCount = static_cast<uint32_t>(instanceDataSet.size());

    // Create an SSBO large enough to hold the world transform matrices of the instances
    BufferOptions bufferOptions = {
        .size = instanceCount * sizeof(glm::mat4),
        .usage = BufferUsageFlags(BufferUsageFlagBits::StorageBufferBit),
        .memoryUsage = MemoryUsage::CpuToGpu // So we can map it to CPU address space
    };
    Buffer buffer = m_device.createBuffer(bufferOptions);

    // Copy in the transform for each instance
    auto bufferData = static_cast<glm::mat4 *>(buffer.map());
    uint32_t instanceIndex = 0;
    for (const auto instanceData : instanceDataSet) {
        std::memcpy(bufferData + instanceIndex,
    ...
    BindGroupOptions bindGroupOptions = {
        ...
        .resources = {{
            .binding = 0,
            .resource = StorageBufferBinding{ .buffer = m_worldTransformBuffers.back()}
        }}
    };
    // clang-format on
    m_worldTransformBindGroups.emplace_back(m_device.createBindGroup(bindGroupOptions));

    return InstanceBindGroupAndCount{
        .instanceBindGroup = m_worldTransformBindGroups.back(),
        .instanceCount = instanceCount
    };
}

void Instancing::calculateWorldTransforms(const tinygltf::Model &model)
{
...
void Instancing::setupMeshNode(const tinygltf::Model &model,
...
void Instancing::resize()
{
    m_opaquePassOptions.depthStencilAttachment.view = m_depthTextureView;
}

void Instancing::updateScene()
{
...
void Instancing::render()
{
    ...
    for (const auto &[pipeline, primitives] : m_pipelinePrimitiveMap) {
        ...
        for (const auto &primitiveData : primitives) {
            ...
            opaquePass.setBindGroup(1, primitiveData.instances.instanceBindGroup);
            ++m_renderStats.setBindGroupCount;
