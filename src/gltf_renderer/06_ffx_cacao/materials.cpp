...
{
    // Allow sampling from Depth Buffer
...
void Materials::initializeScene()
{
    ...
    m_ffxCacao = std::make_unique<FFX_Cacao>(&m_device);
    m_ffxCacao->initializeScene(m_swapchainFormat, m_depthFormat);
    m_ffxCacao->resize(m_window->width(), m_window->height(), m_depthTexture, m_depthTextureView);
}
...
void Materials::updateScene()
{
    ...
    m_ffxCacao->updateScene(m_camera.projectionMatrix(), m_camera.viewMatrix());
}
...
void Materials::render()
{
    ...
    // Render the FFX CACAO post processing effect (it feeds itself off the depthBuffer filled up by the opaque pass)
    m_ffxCacaocommandBuffer = m_ffxCacao->render(m_swapchainViews.at(m_currentSwapchainImageIndex));

    SubmitOptions submitOptions = { .commandBuffers = { m_commandBuffer, m_ffxCacaocommandBuffer },
